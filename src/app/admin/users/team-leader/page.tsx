...
const kpiCounts = {
  ...
  total_interested: mockLeadsData.results.filter(
    (l) => l.status === "Interested"
  ).length,
  total_not_interested: mockLeadsData.results.filter(
    (l) => l.status === "Not Interested"
  ).length,
  total_other_location: mockLeadsData.results.filter(
    (l) => l.status === "Other Location"
  ).length,
  total_not_picked: mockLeadsData.results.filter(
    (l) => l.status === "Not Picked"
  ).length,
};
...
const kpiData = [
  {
    title: "Total Leads",
    valueKey: "total_leads",
    icon: Users,
    color: "text-rose-500",
    link: "/admin/reports/total-leads",
  },
  {
    title: "Total Visit",
    valueKey: "total_visit",
    icon: Eye,
    color: "text-green-500",
    link: "/admin/reports/visit",
  },
  {
    title: "Interested",
  ...
  },
  {
    ...
    icon: XCircle,
    color: "text-red-500",
    link: "/admin/reports/not-interested",
  },
  {
    ...
    link: "/admin/reports/other-location",
  },
  {
    title: "Not Picked",
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-card shadow-2xl rounded-2xl p-0">
        <DialogHeader className="p-6 pb-4 text-center">
          ...
          <DialogDescription>
            Full details for the selected team leader.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="p-4 border-t bg-muted/50 rounded-b-2xl flex-row justify-end gap-2">
          <DialogClose asChild>
            <Button
              type="button"
              variant="outline"
              className="w-full text-foreground hover:bg-primary hover:text-primary-foreground"
            >
              Close
            </Button>
          </DialogClose>
                ...
                <div className="space-y-2 flex-1">
                  ...
                  <div className="relative group">
                    <Input
                      ...
                      className="pl-10 pr-3 py-2 rounded-xl border border-gray-300 focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition-all duration-200 group-hover:border-orange-300"
                    />
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-orange-400 group-hover:text-orange-500 transition-colors" />
                  </div>
                </div>
            ...
            <div className="flex items-center justify-end gap-2 w-full sm:w-auto">
              ...
              <div className="relative w-full sm:w-auto">
                ...
                <Input
                  ...
                  className="pl-10 sm:w-auto lg:w-64 rounded-xl border-gray-300 focus:ring-2 focus:ring-orange-400 focus:border-orange-400"
                />
              </div>

              {/* Filter Button */}
              <Button className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white transition-all rounded-xl">
              ...
              <Button
                ...
                className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white transition-all rounded-xl"
              >
                <PlusCircle className="h-4 w-4" />
                <span className="hidden sm:inline">Add Team Leader</span>
              </Button>
            </div>
          </CardHeader>
                  ...
                  {filteredUsers.length > 0 ? (
                    filteredUsers.map((user, index) => (
                      <React.Fragment key={user.id}>
                        <TableRow>
                          <TableCell>
                            <div className="flex items-center">
                              {/* Show index only on large screens and above */}
                              <span className="hidden lg:inline">
                                {index + 1}
                              </span>

                              {/* Show plus/minus icon on small & medium screens */}
                              <button
                                className="ml-2 inline-flex lg:hidden"
                                onClick={() => toggleExpanded(user.id)}
                              >
                                {expandedRows.has(user.id) ? (
                                  <MinusCircle className="h-6 w-6 text-green-300" />
                                ) : (
                                  <PlusCircle className="h-6 w-6 text-green-300" />
                                )}
                              </button>
                            </div>
                          </TableCell>
                          ...
                          <TableCell className="hidden sm:table-cell">
                            <select
                              className="form-select form-select-sm w-full bg-background border border-input rounded-md px-2 py-1 text-sm"
                              onChange={(e) =>
                                e.target.value &&
                            ...
                            >
                              ...
                              <option value="/admin/reports/interested">
                                Interested
                              </option>
                              <option value="/admin/reports/not-interested">
                                Not Interested
                              </option>
                              <option value="/admin/reports/other-location">
                                Other Location
                              </option>
                              <option value="/admin/reports/total-leads">
                                Lost
                              </option>
                              <option value="/admin/reports/visit">
                                Visit
                              </option>
                            </select>
                          </TableCell>
                        ...
                        {/* Expanded Row - Full Width Premium Card */}
                        {expandedRows.has(user.id) && (
                          <TableRow>
                            <TableCell
                              colSpan={8}
                            ...
                            >
                              <div className="px-4 py-6 md:px-8 md:py-7 bg-gradient-to-b from-muted/60 to-muted/30 rounded-xl mx-2 md:mx-4 my-3 shadow-inner">
                                {/* 2-Column Grid on md+, 1-Column on sm */}
                                ...
                                <div className="mt-6 md:mt-8 flex justify-end border-t pt-5">
                                  <Button
                                    className="w-full sm:w-auto bg-orange-500 hover:bg-orange-600 text-white font-medium px-6"
                                    onClick={() => handleOpenEditForm(user)}
                                  >
                                    <Pencil className="mr-2 h-4 w-4" /> Edit
                                  </Button>
