...
import { TrendingUp, Users, Globe, Briefcase, AreaChart, MoreHorizontal, Filter } from "lucide-react";
...
export default function SuperAdminDashboardPage() {
  const staffChartRef = useRef<HTMLDivElement>(null);
  const sourceChartRef = useRef<HTMLDivElement>(null);

  const handlePrintStaffChart = useReactToPrint({
    content: () => staffChartRef.current,
  });

  const handlePrintSourceChart = useReactToPrint({
    content: () => sourceChartRef.current,
  });
  ...
  const updatedKpiData = kpiData.map(kpi => {
    switch (kpi.title) {
      case "Productivity":
        // No direct match, keep static or derive from data_points if possible
        return kpi;
      case "Staff Management":
        return { ...kpi, value: dashboardData.total_users || 0 };
      case "Source":
        return { ...kpi, value: dashboardData.total_interested || 0 }; // Using total_interested as an example
      case "Freelancers":
        // No direct match, keep static or derive from users array if possible
        return kpi;
      default:
        return kpi;
    }
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={handlePrintStaffChart}>
                  Print
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        <Card ref={sourceChartRef}>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={handlePrintSourceChart}>
                  Print
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
