...
import { SUPERADMIN_SIDENAV_ITEMS } from '@/lib/constants';
...
const NavItem = ({ item, isActive, isCollapsed }: { item: any; isActive: boolean; isCollapsed: boolean; }) => {
                  ...
                  <DropdownMenuTrigger asChild>
                    <div className="w-full">
                       <Button variant="ghost" className={cn(
                          ...
                          isAnySubItemActive && 'bg-primary text-primary-foreground hover:bg-primary/90 hover:text-primary-foreground'
                        )}>
                          {item.icon}
  ...
  if (item.submenu) {
    return (
      <Collapsible defaultOpen={isAnySubItemActive}>
        <CollapsibleTrigger
          className={cn(
...
const SidebarContent = ({ isCollapsed }: { isCollapsed: boolean }) => {
  ...
  const isActive = (path: string) => {
    if (path === '/superadmin') return pathname === path;
    const item:any = SUPERADMIN_SIDENAV_ITEMS.find(item => item.path === path);
    if (item?.submenu) {
        return item.subMenuItems.some((sub:any) => pathname.startsWith(sub.path));
    }
  ...
  return (
    <ul className="space-y-2">
      {SUPERADMIN_SIDENAV_ITEMS.map((item) => (
          <NavItem key={item.title} item={item} isActive={isActive(item.path)} isCollapsed={isCollapsed} />
      ))}
    </ul>
  );
};
...
export function SuperAdminSidebar({ isSidebarOpen, setSidebarOpen, isCollapsed, setIsCollapsed }: { 
